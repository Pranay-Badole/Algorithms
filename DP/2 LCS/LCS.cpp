...
int lcs_memoized(string x, string y, int n, int m, std::vector<std::vector<int>> &dp){
	if(m==0 || n==0){
		return 0;
	}
	...
	if(x[n-1] == y[m-1]){
		return dp[n][m] = (1 + lcs_memoized(x, y, n-1, m-1, dp));
	}
	else{
		return dp[n][m] = max(lcs_memoized(x, y, n-1, m, dp),lcs_memoized(x, y, n, m-1, dp));
	}
}
...
void solve(){	 
	...
	int ans = lcs_recursive(x, y, n, m);
	// int ans = lcs_memoized(x, y, n, m, dp);
